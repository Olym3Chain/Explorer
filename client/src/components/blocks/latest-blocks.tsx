import { useQuery } from "@tanstack/react-query";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";
import { formatTime, formatNumber } from "@/lib/formatters";
import { Box, ExternalLink } from "lucide-react";
import { Link } from "wouter";
import type { Block } from "@shared/schema";

export function LatestBlocks() {
  const { data: blocks, isLoading } = useQuery<Block[]>({
    queryKey: ["/api/blocks/latest"],
  });

  if (isLoading) {
    return (
      <Card className="border-border shadow-lg">
        <CardHeader>
          <CardTitle className="flex items-center">
            <Box className="h-5 w-5 olym-primary mr-2" />
            Latest Blocks
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="flex items-center justify-between p-4 bg-muted rounded-lg">
                <div className="flex items-center space-x-4">
                  <Skeleton className="h-10 w-10 rounded-full" />
                  <div>
                    <Skeleton className="h-4 w-20 mb-2" />
                    <Skeleton className="h-3 w-16" />
                  </div>
                </div>
                <div className="text-right">
                  <Skeleton className="h-4 w-12 mb-2" />
                  <Skeleton className="h-3 w-16" />
                </div>
              </div>
            ))}
          </div>
        </CardContent>
      </Card>
    );
  }

  if (!blocks || blocks.length === 0) {
    return (
      <Card className="border-border shadow-lg">
        <CardHeader>
          <CardTitle className="flex items-center">
            <Box className="h-5 w-5 olym-primary mr-2" />
            Latest Blocks
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-center py-8">
            <p className="text-muted-foreground">No blocks found</p>
          </div>
        </CardContent>
      </Card>
    );
  }

  return (
    <Card className="border-border shadow-lg">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center">
            <Box className="h-5 w-5 olym-primary mr-2" />
            Latest Blocks
          </CardTitle>
          <Link href="/blocks" className="text-sm olym-primary hover:underline flex items-center">
            View all <ExternalLink className="h-3 w-3 ml-1" />
          </Link>
        </div>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          {blocks.slice(0, 5).map((block) => (
            <Link key={block.id} href={`/block/${block.blockNumber}`}>
              <div className="flex items-center justify-between p-4 bg-muted rounded-lg hover:bg-muted/80 transition-colors cursor-pointer">
                <div className="flex items-center space-x-4">
                  <div className="w-10 h-10 bg-blue-500 rounded-full flex items-center justify-center">
                    <Box className="h-4 w-4 text-white" />
                  </div>
                  <div>
                    <div className="font-semibold text-foreground">
                      #{formatNumber(block.blockNumber)}
                    </div>
                    <div className="text-sm text-muted-foreground">
                      {formatTime(block.timestamp)}
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-sm font-medium text-foreground">
                    {formatNumber(block.transactionCount)} txns
                  </div>
                  <div className="text-xs text-muted-foreground">in 2 secs</div>
                </div>
              </div>
            </Link>
          ))}
        </div>
      </CardContent>
    </Card>
  );
}
